...
import { Crown, Trophy, Users, ChevronRight } from 'lucide-react';
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-1 text-muted-foreground">
            ...
            <span>Entrada: {formatPrize(pool.entry_fee)}</span>
          </div>
          ...
          <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-amber-600 dark:text-amber-400">
                {formatPrize(pool.total_accumulated ?? 0)}
              </span>
            </div>
          </div>
        )}
        ...
        <Button asChild className="w-full group-hover:bg-amber-500 group-hover:text-amber-950 transition-colors">
