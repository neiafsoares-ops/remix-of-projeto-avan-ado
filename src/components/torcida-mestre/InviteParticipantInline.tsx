...
interface InviteParticipantInlineProps {
  poolId: string;
  roundId: string;
  entryFee: number;
...
export function InviteParticipantInline({ 
  poolId, 
  roundId, 
  entryFee,
  existingParticipants,
  onSuccess 
}: InviteParticipantInlineProps) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          status: 'active', // Admin adding directly = approved
          ...
          <div className="space-y-3">
            ...
            {searchedUser.isAlreadyParticipant ? (
              <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 p-2 rounded-lg">
                ...
                <span>Já é participante desta rodada</span>
              </div>
            ) : (
              <Button 
